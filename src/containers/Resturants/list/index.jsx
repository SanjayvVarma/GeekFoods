        ...
        <div className='resturants' >
            ...
            <ul className='rest-ul'>
                {currentItems.map((restaurant) => (
                    <li className='rest-li' key={restaurant.name}>
                        <strong className='rest-strong'>{restaurant.name}<span style={{ color: 'yellow', fontSize: '1.6rem' }}>{renderRatingStars(restaurant.rating)}</span></strong>
            ...
            <div className='rest-div-btn'>
                {Array.from({ length: totalPages }, (_, index) => index + 1).map((page) => (
                    <button className='rest-button' key={page} onClick={() => handlePageChange(page)}>
                        {page}
                    </button>
                ))}
            </div>
